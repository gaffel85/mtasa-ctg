...
function getOrCreatePlotWindow()
    if not plotWindow then
        ...
        addEventHandler ( "onDgsMouseClickUp", xScalePlusButton, function() 
            xScale = xScale + scaleSteps
            outputConsole("X Scale: "..xScale)
            plotLocations()
        end)
        addEventHandler ( "onDgsMouseClickUp", xScaleMinusButton, function() 
            xScale = xScale - scaleSteps
            outputConsole("X Scale: "..xScale)
            plotLocations()
        end)
        addEventHandler ( "onDgsMouseClickUp", yScalePlusButton, function() 
            yScale = yScale + scaleSteps
            outputConsole("Y Scale: "..yScale)
            plotLocations()
        end)
        addEventHandler ( "onDgsMouseClickUp", yScaleMinusButton, function() 
            yScale = yScale - scaleSteps
            outputConsole("Y Scale: "..yScale)
            plotLocations()
        end)
        addEventHandler ( "onDgsMouseClickUp", xOffsetPlusButton, function() 
            xOffset = xOffset + offsetSteps
            outputConsole("X Offset: "..xOffset)
            plotLocations()
        end)
        addEventHandler ( "onDgsMouseClickUp", xOffsetMinusButton, function() 
            xOffset = xOffset - offsetSteps
            outputConsole("X Offset: "..xOffset)
            plotLocations()
        end)
        addEventHandler ( "onDgsMouseClickUp", yOffsetPlusButton, function() 
            yOffset = yOffset + offsetSteps
            outputConsole("Y Offset: "..yOffset)
            plotLocations()
        end)
        addEventHandler ( "onDgsMouseClickUp", yOffsetMinusButton, function() 
            yOffset = yOffset - offsetSteps
            outputConsole("Y Offset: "..yOffset)
            plotLocations()
        end)
        addEventHandler ( "onDgsMouseClickUp", reduceLocationsButton, function() 
            outputChatBox("Reducing locations")
            callReducer()
        end)
        addEventHandler ( "onDgsMouseClickUp", saveLocationsButton, function() 
            outputChatBox("Saving locations")
            saveWholeFileAsJson()
        end)
        addEventHandler ( "onDgsMouseClickUp", loadLocationsButton, function() 
